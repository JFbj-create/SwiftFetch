...
> 融合 **Aria2 静态分块稳定机制** 与 **IDM 动态分片负载调度** 优势的 Rust CLI 无 UI 下载引擎。外层静态基底分块 (Aria2 风)，内层慢块自适应动态拆分子分片 (IDM 工作窃取思想)，配合 Tokio 多线程异步任务调度与 TEMA 平滑网速算法，实现高稳定性与高带宽利用率的平衡。
...
| 层级 | 策略 | 思想来源 | 说明 |
|---|---|---|---|
| **外层 · 静态基底块 (Base Chunk)** | **固定大小静态分块** (按文件大小自适应 1MB/2MB/4MB/8MB) | Aria2 | 稳定的断点快照边界，减少 HTTP 请求数量，避免 Range 爆炸 |
| **内层 · 动态子分片 (Sub Chunk)** | **仅对速度滞后的慢基底块内部自适应动态拆分** | IDM 工作窃取 | 快块保持静态不扰动，慢块按 `慢度指数` 动态切 N 个子分片派给空闲 Worker 抢跑 |
| **最小子分片阈值** | 硬性下限 256 KB | — | 防止动态切分过度 → HTTP 请求数爆炸 (IDM 常见缺陷) |
...
- **工作窃取 (Work Stealing)**：空闲 Worker 主动从慢基底块的子分片队列抢任务 (避免 Aria2 静态分片长尾阻塞)
